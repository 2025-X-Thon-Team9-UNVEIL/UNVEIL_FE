...
export default function Button({
  ...
  return (
    <button
      type={type}
      disabled={disabled}
      className={cn('inline-flex h-[45px] w-[355px] items-center justify-center rounded-xl', BgColor[color], className)}
      {...props}>
      <Txt weight={textWeight} className={cn('text-xl text-white', textClassName)}>
        {children}
      </Txt>
    </button>
  );
